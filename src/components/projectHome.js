...
export default function ProjectHome({ projects }) {
  ...
  const handleProj = (side) => {
    const cardProj = document.getElementById("b");
    if (side == "btn-left-proj") {
      if (cardProj.className.includes("moveleft")) {
        cardProj.className = cardProj.className.replace(
          "moveleft",
          "move-left"
        );
  ...
  return (
    <div className="home-section">
      ...
      <div className="wheel">
        <button
          type=""
          className="btn-wheel btn-back"
          onClick={() => handleProj("btn-left-proj")}
          id="btn-left-proj"
        ></button>

        <div className="ball-container">
          <div className="progresive-bar" id="bar-proj"></div>
        </div>

        <button
          type=""
          className="btn-wheel btn-next"
          onClick={() => handleProj("btn-right-proj")}
          id="btn-right-proj"
        ></button>
      </div>
        ...
        <div className="card-header">
          <Link href={`/projects/${myProj._id}`}>
            <a href="" className="a-push">
              {" "}
              <h2 className="push">{myProj.title}</h2>{" "}
            </a>
          </Link>
        </div>
