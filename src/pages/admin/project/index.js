...
export default function AdminProj(props) {
  ...
  const reqDelete = async (id) => {
    ...
    if (res.status == 200) {
      const deleteproj = (document.getElementById(res.data.iid).style.display =
        "none");
    }
  };
        ...
        {projs.length > 0 ? (
          projs.map((proj) => (
            <div className="card card-home" key={proj._id} id={proj._id}>
              ...
              <div className="card-description">{proj.description}</div>
              <div className="card-links">
                {proj.github != undefined && proj.github != "" && (
                  <a className="card-link" href={proj.github}>
                    {"-"} Github
                  </a>
                )}
                {proj.website != undefined && proj.website != "" && (
                  <a className="card-link" href={proj.website}>
                    {"-"} Website{" "}
                  </a>
                )}
              </div>
              <div className="btn-container">
                ...
                <button
                  onClick={() => reqDelete(proj._id)}
                  className="btn-admin btn-delete push"
                >
                  DELETE
                </button>
              </div>
